...
function EditKnowledgeBaseDialogForm({
  ...
  return (
    <motion.div
      key={kb.id}
      className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-foreground/40"
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      >
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          ID: <span className="font-mono text-muted-foreground">{kb.id}</span>
        </p>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button
            ...
            className="px-4 py-2 text-sm font-medium rounded-xl text-primary-foreground bg-primary hover:opacity-90 transition-colors disabled:opacity-50"
          >
            {pending ? "请稍候…" : "保存"}
          </button>
        </div>
