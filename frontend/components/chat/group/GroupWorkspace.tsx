    ...
    <div className="flex flex-1 flex-col min-h-0 overflow-y-auto no-scrollbar px-4 md:px-8">
      <div className="mx-auto w-full max-w-3xl lg:max-w-4xl xl:max-w-5xl pb-8 pt-6 md:pt-10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-5">
          <div className="flex h-[3.5rem] w-[3.5rem] shrink-0 items-center justify-center rounded-2xl border border-border bg-surface shadow-sm">
        ...
        <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <button
            type="button"
            disabled
            className="rounded-xl border border-[#ebebeb] bg-surface p-4 text-left opacity-[0.92] shadow-[0_1px_0_rgba(0,0,0,0.03)]"
          >
            <div className="text-sm font-medium text-foreground">添加指令</div>
          ...
          <button
            ...
            className="rounded-xl border border-[#ebebeb] bg-surface p-4 text-left opacity-[0.92] shadow-[0_1px_0_rgba(0,0,0,0.03)]"
          >
            <div className="text-sm font-medium text-foreground">资料库</div>
            <div className="mt-1 text-xs leading-relaxed text-muted-foreground">
              为分组挂载知识片段（后续版本）
            </div>
          </button>
        ...
        <section className="mt-12">
          ...
          {n === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border/90 bg-muted/80 px-6 py-20 text-center">
              <FolderOpen
              ...
              {conversationsInGroup.map((c) => (
                <li key={c.id}>
                  <button
                    ...
                    className="flex w-full min-h-[3rem] items-center justify-between gap-3 rounded-xl border border-transparent bg-surface px-4 py-3 text-left text-sm font-medium text-foreground shadow-sm ring-1 ring-[#e8e8e8] transition hover:bg-muted/90 hover:ring-gray-300/80"
                  >
